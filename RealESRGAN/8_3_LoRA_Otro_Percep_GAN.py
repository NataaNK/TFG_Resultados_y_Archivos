...
class SimpleDiscriminator(nn.Module):
    def __init__(self, in_channels=3, base_channels=64):
        ...
        self.model = nn.Sequential(
            ...
            nn.LeakyReLU(0.2, inplace=True),
            nn.utils.spectral_norm(nn.Conv2d(base_channels*4, 1, 3, stride=1, padding=1))
        )
    def forward(self, x):
        return self.model(x)
...
def main():
    ...
    parser.add_argument("--lambda_gan", type=float, default=0.1, help="Factor de ponderación para la pérdida GAN")
...
# --- Definición del discriminador simple ---
class SimpleDiscriminator(nn.Module):
    def __init__(self, in_channels=3, base_channels=64):
        super(SimpleDiscriminator, self).__init__()
        self.model = nn.Sequential(
...
# --- Definición de EdgeLoss (ya incluida anteriormente) ---
class EdgeLoss(nn.Module):
    def __init__(self):
        super(EdgeLoss, self).__init__()
        kernel_x = torch.tensor([[1, 0, -1],
