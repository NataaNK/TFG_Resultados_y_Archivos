...
# Importar modelo VGG19 para referencia (no se usa en este script)
...
def main():
    parser = argparse.ArgumentParser(description="Fine-tune Real-ESRGAN x4v3 con LoRA y GAN Loss (como perceptual)")
    ...
    # GAN loss weight (se usará como 'perceptual' en este finetune)
    parser.add_argument("--lambda_gan", type=float, default=0.1, help="Factor de ponderación para la GAN Loss")
    ...
    # --- Configurar la pérdida L1 y la GAN loss para el generador ---
    criterion = nn.L1Loss()
    ...
    for epoch in range(1, args.epochs+1):
        ...
        for lr_imgs, hr_imgs in train_loader:
            ...
            with torch.cuda.amp.autocast():
                ...
                g_loss = loss_l1 + lambda_gan * gan_loss
        ...
        print(f"Epoch {epoch}/{args.epochs} - Pérdida Total: {epoch_loss:.4f} (L1: {avg_l1_loss:.4f}, GAN: {avg_gan_loss:.4f}). Tiempo: {elapsed:.2f} s")
