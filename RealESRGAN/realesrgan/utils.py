...
############################################
# NUEVAS FUNCIONES PARA APLICAR LoRA EN INFERENCE
############################################

# Función para convertir una capa Conv2d en una versión LoRA usando loralib
def convert_conv2d_to_lora(conv_module, r, lora_alpha):
    # Se extrae el tamaño del kernel (asumiendo kernel cuadrado)
    ...
    # Copiar pesos y bias del módulo original al nuevo módulo LoRA
    new_conv.conv.weight.data.copy_(conv_module.weight.data)
    if conv_module.bias is not None:
        new_conv.conv.bias.data.copy_(conv_module.bias.data)
    return new_conv

# Función recursiva para reemplazar todas las capas Conv2d en un módulo por sus versiones LoRA
def replace_conv_with_lora(module, r=8, lora_alpha=8):
    if len(list(module.named_children())) == 0:
        return
    for name, child in module.named_children():
        if isinstance(child, torch.nn.Conv2d) and not isinstance(child, lora.Conv2d):
...
class RealESRGANer():
    ...
    def __init__(self,
        ...
        # Cargar modelo (se asume que "model" se pasa ya instanciado)
        if isinstance(model_path, list):
            # DNI: interpolación entre dos checkpoints
        ...
        # ---- MODIFICACIÓN: Reemplazar las capas Conv2d por las versiones LoRA antes de cargar los pesos ----
        replace_conv_with_lora(model, r=8, lora_alpha=8)
        # ----------------------------------------------------------------------------------------------

        # Cargar el state_dict usando la clave "params_ema" o "params"
    ...
    def pre_process(self, img):
        """Pre-process: pre-pad and mod pad so that image dimensions are divisible."""
        img = torch.from_numpy(np.transpose(img, (2, 0, 1))).float()
        self.img = img.unsqueeze(0).to(self.device)
        if self.half:
            self.img = self.img.half()
