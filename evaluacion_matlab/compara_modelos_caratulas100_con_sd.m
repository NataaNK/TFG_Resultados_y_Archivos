...
results     = cell(num_metrics, num_models);  % Usamos celdas para almacenar media y desviación
...
for model_idx = 1:num_models
    ...
    psnr_accum    = []; psnr_count    = 0;
    ssim_accum    = []; ssim_count    = 0;
    ms_ssim_accum = []; ms_ssim_count = 0;
    fsim_accum    = []; fsim_count    = 0;
    
    brisque_accum = []; brisque_count = 0;
    piqe_accum    = []; piqe_count    = 0;
    ...
    % ============================================================
    % 7) Calcular los promedios y desviaciones estándar para cada métrica en este dataset
    % ============================================================
    % Evitamos la división por 0 con if-else:
    
    % PSNR
    if psnr_count > 0
        avg_psnr = mean(psnr_accum);
        sd_psnr = std(psnr_accum);
    else
        avg_psnr = 0;
        sd_psnr = 0;
    end
    ...
    % SSIM
    if ssim_count > 0
        avg_ssim = mean(ssim_accum);
        sd_ssim = std(ssim_accum);
    else
        avg_ssim = 0;
        sd_ssim = 0;
    end
    ...
    % MS-SSIM
    if ms_ssim_count > 0
        avg_msssim = mean(ms_ssim_accum);
        sd_msssim = std(ms_ssim_accum);
    else
        avg_msssim = 0;
        sd_msssim = 0;
    end
    ...
    % FSIM
    if fsim_count > 0
        avg_fsim = mean(fsim_accum);
        sd_fsim = std(fsim_accum);
    else
        avg_fsim = 0;
        sd_fsim = 0;
    end
    ...
    % BRISQUE
    if brisque_count > 0
        avg_brisque = mean(brisque_accum);
        sd_brisque = std(brisque_accum);
    else
        avg_brisque = 0;
        sd_brisque = 0;
    end
    ...
    % PIQE
    if piqe_count > 0
        avg_piqe = mean(piqe_accum);
        sd_piqe = std(piqe_accum);
    else
        avg_piqe = 0;
        sd_piqe = 0;
    end
    
    % Almacenamos los resultados en la matriz, en formato media (sd: desviación)
    results{1, model_idx} = sprintf('%.3f (sd: %.3f)', avg_psnr, sd_psnr);
    results{2, model_idx} = sprintf('%.3f (sd: %.3f)', avg_ssim, sd_ssim);
    results{3, model_idx} = sprintf('%.3f (sd: %.3f)', avg_msssim, sd_msssim);
    results{4, model_idx} = sprintf('%.3f (sd: %.3f)', avg_fsim, sd_fsim);
    results{5, model_idx} = sprintf('%.3f (sd: %.3f)', avg_brisque, sd_brisque);
    results{6, model_idx} = sprintf('%.3f (sd: %.3f)', avg_piqe, sd_piqe);
end
