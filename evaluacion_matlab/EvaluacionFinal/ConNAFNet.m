...
times_sec = [581.07, 178.35, 778.48, 285.08, 78.36, 718.95];
...
subdataset_template = {
    'C:\Users\nekos\OneDrive\Escritorio\NAFNet\DRTC_Final150\Final150Noise'
};
...
for model_idx = 1:num_models
    ...
    for sub_idx = 1:length(dataset_paths)
        ...
        for i = 1:length(file_list)
            ...
            ref_files = dir(fullfile(current_ref_dir, '*.*')); % Listamos todos los archivos en la carpeta de referencia
            pattern = sprintf('^%d_%s.*', img_index, keyword); % Por ejemplo, para img_index=1 y keyword='BN': '1_BN.*'
