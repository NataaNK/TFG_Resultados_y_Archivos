function compara_modelos_caratulas100_visualizacion_v1()
    ...
    for k = 1:length(lr_files)
        ...
        title(sprintf('LR/Deformed (%s)\nPSNR=%.3f, SSIM=%.3f', ...
            deform_str, psnr_lr, ssim_lr), 'Interpreter','none');
        ...
        for m = 1:length(model_dirs)
            ...
            % Métricas vs. ref
            [psnr_val, ssim_val] = deal(NaN);

            if has_reference
                try
                    ref_gray = rgb2gray(ref_img);
                    proc_model = rgb2gray(model_img);
            ...
            title(sprintf('%s\nPSNR=%.3f, SSIM=%.3f', ...
                model_name, psnr_val, ssim_val), 'Interpreter','none');
        end
    end
end
