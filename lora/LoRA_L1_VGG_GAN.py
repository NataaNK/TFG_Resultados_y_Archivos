...
# Función para convertir capas Conv2d a su versión LoRA
...
##########################################
# Perceptual Loss basada en VGG19
##########################################
...
##########################################
# Simple Discriminator y GAN Loss
##########################################
...
##########################################
def main():
    parser = argparse.ArgumentParser(description="Finetune Real-ESRGAN x4v3 con LoRA, L1, Perceptual y GAN Loss")
    ...
    # Ponderaciones para las pérdidas
    parser.add_argument("--lambda_perceptual", type=float, default=1.0, help="Peso de la pérdida perceptual")
    parser.add_argument("--lambda_gan", type=float, default=0.1, help="Peso de la GAN Loss")
    ...
    # Configurar el discriminador y la GAN Loss
    ...
    # Configurar las pérdidas del generador: L1, Perceptual y GAN Loss
    criterion = nn.L1Loss()
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for lr_imgs, hr_imgs in train_loader:
            ...
            with torch.cuda.amp.autocast():
                ...
                g_loss = loss_l1 + lambda_perceptual * loss_perc + lambda_gan * gan_loss
            scaler.scale(g_loss).backward()
            scaler.step(optimizer)
            scaler.update()

            batch_size = lr_imgs.size(0)
            epoch_loss += g_loss.item() * batch_size
        ...
        avg_gan_loss = total_gan_loss / len(train_dataset)

        elapsed = time.time() - start_time
        print(f"Epoch {epoch}/{args.epochs} - Pérdida Total: {epoch_loss:.4f} (L1: {avg_l1_loss:.4f}, VGG: {avg_perc_loss:.4f}, GAN: {avg_gan_loss:.4f}). Tiempo: {elapsed:.2f} s")


        # Calcular métricas de entrenamiento sobre el propio conjunto de entrenamiento
