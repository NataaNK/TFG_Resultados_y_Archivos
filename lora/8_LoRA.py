...
def compute_piqe(np_img):
    # Asegurarse de que la imagen esté en rango [0, 255] y de tipo uint8
    if np_img.max() <= 1.0:
...
# --- Función para paddear tensores a tamaño máximo en el batch
# Modificada para que el tamaño sea múltiplo del factor de upscale (4)
def pad_to_max(tensors, upscale=4):
    max_h = max(t.shape[1] for t in tensors)
    max_w = max(t.shape[2] for t in tensors)
    # Redondear al múltiplo superior de upscale
...
def validate(model, dataloader, device):
    ...
    with torch.no_grad():
        for lr_imgs, hr_imgs in dataloader:
            ...
            sr_imgs = model(lr_imgs)  # Salida en rango [0,1]
...
class PerceptualLoss(nn.Module):
    ...
    def forward(self, sr, hr):
        # Mover mean y std al dispositivo de las entradas
...
def main():
    parser = argparse.ArgumentParser(description="Fine-tune Real-ESRGAN x4v3 con LoRA para mejorar PSNR y PIQE")
    ...
    # Nuevo argumento para ponderar la pérdida perceptual
    ...
    # Crear datasets y dataloaders (usamos collate_fn para que todos los tensores tengan el mismo tamaño)
    ...
    # --- Cargar pesos pre-entrenados e interpolar (DNI) ---
    ...
    # Inicializar el modelo y cargar los pesos combinados
    ...
    # --- Reemplazar capas Conv2d por loralib.Conv2d ---
    ...
    criterion = nn.L1Loss()
    perceptual_criterion = PerceptualLoss(feature_layer=35)  # extrae características de una capa intermedia
    perceptual_criterion = perceptual_criterion.to(device)
    lambda_perceptual = args.lambda_perceptual
    optimizer = optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=args.lr)

    # Inicializamos el GradScaler para mixed precision
    ...
    for epoch in range(1, args.epochs+1):
        ...
        for lr_imgs, hr_imgs in train_loader:
            ...
            hr_imgs = hr_imgs.to(device)
            optimizer.zero_grad()
            # Usamos autocast para entrenamiento en precisión mixta
            with torch.cuda.amp.autocast():
                sr_imgs = model(lr_imgs)
                loss_l1 = criterion(sr_imgs, hr_imgs)
                loss_perc = perceptual_criterion(sr_imgs, hr_imgs)
                loss = loss_l1 + lambda_perceptual * loss_perc
            # Escalar la pérdida y realizar backward
            scaler.scale(loss).backward()
            scaler.step(optimizer)
            scaler.update()

            batch_size = lr_imgs.size(0)
            total_l1_loss += loss_l1.item() * batch_size
            total_perc_loss += loss_perc.item() * batch_size

        epoch_loss /= len(train_dataset)
        avg_l1_loss = total_l1_loss / len(train_dataset)
        avg_perc_loss = total_perc_loss / len(train_dataset)

        elapsed = time.time() - start_time
        print(f"Epoch {epoch}/{args.epochs} - Pérdida Total: {epoch_loss:.4f} (L1: {avg_l1_loss:.4f}, VGG: {avg_perc_loss:.4f}. Tiempo: {elapsed:.2f} s")

        # Calcular métricas sobre el conjunto de entrenamiento
        train_psnr, train_piqe = validate(model, train_loader, device)
        print(f"  -> Entrenamiento: PSNR = {train_psnr:.2f} dB, PIQE = {train_piqe:.2f}")
